...
namabot = 'Fiin-Botz'
namaowner = 'Fiin'
nomorowner = '6289519241989'
githubowner = 'https://github.com/Fiin24'
youtubeowner = 'https://youtube.com/c/Fiin'
...
module.exports = Fiin = async (Fiin, mek) => {
		try {
...
cmhit.push(command)
Fiin.on("CB:action,,battery", json => {
...
//━━━━━━━━━━━━━━━[ FUNCTION 2 ]━━━━━━━━━━━━━━━\\
			...
			const botNumber = Fiin.user.jid
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			pushname = Fiin.contacts[sender] != undefined ? Fiin.contacts[sender].vname || Fiin.contacts[sender].notify : undefined
			const groupMetadata = isGroup ? await Fiin.groupMetadata(from) : ''
			...
			const group = Fiin.chats.array.filter(v => v.jid.endsWith('g.us'))
            const private = Fiin.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
            const totalchat = await Fiin.chats.all()
...
//━━━━━━━━━━━━━━━[ DATE ]━━━━━━━━━━━━━━━\\
        ...
        const isUrl = (url) => {
			...
			const reply = (teks) => {
			Fiin.sendMessage(from, teks, text, { thumbnail: gambar, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} WIB - ${week}`,body:"Developer Fiin-Botz",previewType:"PHOTO",thumbnail:gambar,sourceUrl:`https://chat.whatsapp.com/HVD9ZVdjUOZI5m33ClCv1o`}}})
		}
		const reply2 = (teks) => {
Fiin.sendMessage(from, teks, text, {quoted: mek, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})
}
			const sendMess = (hehe, teks) => {
				Fiin.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? Fiin.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Fiin.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			if (budy.includes("https://chat.whatsapp.com/")) {
				...
				setTimeout(() => {
				Fiin.groupRemove(from, [kic]).catch((e) => { reply(`Jadikan Bot Sebagai Admin Untuk Menggunakan Perintah Ini!!`) })
				}, 0)
			   }
//━━━━━━━━━━━━━━━[ WAKTU ]━━━━━━━━━━━━━━━\\
			            var ase = new Date();
...
//━━━━━━━━━━━━━━━[ FAKE REPLY ]━━━━━━━━━━━━━━━\\
           ...
           const fakedoc = {key: {fromMe: false, participant: `${nomorowner}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {documentMessage: {mimetype: 'application/octet-stream', title: `${ucapanFakereply}`, pageCount: 0, fileName: `Fiin-Botz by Fiin`, jpegThumbnail: fs.readFileSync(`./media/gambar/thumb.jpg`)}}}
...
const ftroly = {key : {fromMe:false,
	participant : '0@s.whatsapp.net'},
       message: { 
              orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `Fiin-Botz by Fiin`, 
                            orderTitle: `${namaowner}`,
                            thumbnail: thumb, 
                            sellerJid: '0@s.whatsapp.net',
                            contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true}}}
			const fakeitem = (teks) => {
           Fiin.sendMessage(from, teks, text, {
                   quoted: {
                           key:{
                 	       fromMe:false, 
                           participant:`0@s.whatsapp.net`, ...(from ? {
    ...
    Fiin.sendMessage(from, buttonMessagesI, MessageType.buttonsMessage, {quoted: mek})
  }
    const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    Fiin.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
      ...
      kma = gam1;
      mhan = await Fiin.prepareMessage(from, kma, video);
      ...
      Fiin.sendMessage(id, buttonMessages, MessageType.buttonsMessage, {quoted: mek})
    };
        
        const sendButloc = async(id, text1, desc1, gam1, but = [], options = {}) => {
               let kma = gam1
               Fiin.sendMessage(id, {"contentText": text1,
               "footerText": desc1, 
               "buttons": but,
               "headerType": "LOCATION",
                       "locationMessage": {
      ...
      Fiin.sendMessage(id, buttonMessage, MessageType.buttonsMessage, {quoted: mek})
    };
...
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
			...
			let authorname = Fiin.contacts[from] != undefined ? Fiin.contacts[from].vname || Fiin.contacts[from].notify : undefined	
			if (authorname != undefined) { } else { authorname = groupName }	
			
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Fiin-Botz'; if (!author) author = 'by Fiin';	
...
async function sendButLocation(id, text1, desc1, gam1, but = [], options = {}) {
            ...
            return Fiin.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        }       
        
        const sendFileFromUrl = async(link, type, options) => {
           hasil = await getBuffer(link)
	       Fiin.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       Fiin.sendMessage(from, hasil, type, options).catch(e => {
	       Fiin.sendMessage(from, { url : link }, type, options).catch(e => {
...
Fiin.updatePresence(from, Presence.composing)
...
run = process.uptime() 
           Fiin.setStatus(`${namabot} Aktif Selama ${kyun(run)} | Tiktok : Fiin`).catch((_)=>_);
...
const fakeText = (teks) => {
Fiin.sendMessage(from, teks, text, {quoted: froxx})
}
...
stst = await Fiin.getStatus(`${sender.split('@')[0]}@c.us`)
...
╾ _Browser : ${Fiin.browserDescription[1]}_
╾ _Server : ${Fiin.browserDescription[0]}_
╾ _Version : ${Fiin.browserDescription[2]}_
╾ _Handphone : ${Fiin.user.phone.device_manufacturer}_
...
//━━━━━━━━━━━━━━━[ ANIME FITUR ]━━━━━━━━━━━━━━━\\
            ...
            if (isBanned) return reply(mess.banned)
             ...
             cewe = await getBuffer(anu.result.female)
              ...
              Fiin.sendMessage(from, cowo, image, {quoted: froxx, caption: 'Cowo' })
              Fiin.sendMessage(from, cewe, image, {quoted: froxx, caption: 'Cewe' })
              break
             case 'waifu':
             case 'loli':
            case 'husbu':
            ...
            if (isBanned) return reply(mess.banned)
              ...
              imageMsg = ( await Fiin.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'©Created by Fiin', imageMessage: imageMsg,
              contentText:`NthnDevId It's Me`,buttons,headerType:4}
              prep = await Fiin.prepareMessageFromContent(from,{buttonsMessage},{quoted: froxx})
              FiinrelayWAMessage(prep)
...
[ LINK YOUTUBE ]
• https://youtube.com/Fiin`
but = [
          { buttonId: `${prefix}menu`, buttonText: { displayText: 'MENU' }, type: 1 }
]
        sendButLocation(from, teks, `©Created by Fiin`, fakeimg, but, { thumbnail: Buffer.alloc(0) })
break
...
//━━━━━━━━━━━━━━━[ ANIMATION ANIMALS ]━━━━━━━━━━━━━━━\\
                   ...
                   Fiin.sendMessage(from, anu1, image, {caption: `Ini Rubah`, quoted: froxx})
                   break
                   case 'dog':  
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/dog`)
                   anu1 = await getBuffer(anu.link)
                   Fiin.sendMessage(from, anu1, image, {caption: `Ini Anjing`, quoted: froxx})
                   break
                   case 'cat':
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/cat`)
                   anu1 = await getBuffer(anu.link)
                   Fiin.sendMessage(from, anu1, image, {caption: `Ini Kucing`, quoted: froxx})
                   break
                   case 'panda':  
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/panda`)
                   anu1 = await getBuffer(anu.link)
                   Fiin.sendMessage(from, anu1, image, {caption: `Ini Panda`, quoted: froxx})
                   break
                   case 'panda2':  
                   if (isBanned) return reply(mess.banned)
                   ...
                   anu1 = await getBuffer(anu.link)
                   Fiin.sendMessage(from, anu1, image, {caption: `Ini Burung`, quoted: froxx})
                   break
                   case 'koala':  
                   if (isBanned) return reply(mess.banned)
                   anu = await fetchJson(`https://some-random-api.ml/img/koala`)
                   anu1 = await getBuffer(anu.link)
                   Fiin.sendMessage(from, anu1, image, {caption: `Ini Koala`, quoted: froxx})
                   break
...
//━━━━━━━━━━━━━━━[ RANDOM IMAGE ]━━━━━━━━━━━━━━━\\
                ...
                reply(mess.wait)
                    getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${Lolhuman}`).then((gambar) => {
                        Fiin.sendMessage(from, gambar, image, { quoted: froxx })
                    })
                    break
                case 'chiisaihentai':
                case 'trap':
...
//━━━━━━━━━━━━━━━[ KODE BAHASA ]━━━━━━━━━━━━━━━\\
                     ...
                     case 'bahasa':
                     if (isBanned) return reply(mess.banned)
                    Fiin.sendMessage(from, bahasa(), text, { quoted:froxx })
                    break 
                    case 'kodenegara':
                    if (isBanned) return reply(mess.banned)
					Fiin.sendMessage(from, negara(), text)
					break
                    case 'tts':
                    if (isBanned) return reply(mess.banned)
				    if (args.length < 1) return Fiin.sendMessage(from, 'Masukan Kode Bahasanya!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return Fiin.sendMessage(from, 'Masukan Text Nya?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 500
...
break
                case 'premiumlist':
				Fiin.updatePresence(from, Presence.recording) 
				teks = 'LIST USER PREMIUM :\n'
				for (let premm of prem) {
					teks += `~> @${premm.split('@')[0]}\n`
					}
					teks += `Total : ${prem.length}`
				Fiin.sendMessage(from, teks.trim(), extendedText, {quoted: froxx, contextInfo: {"mentionedJid": prem}})
				break
//━━━━━━━━━━━━━━━[ BANNED ]━━━━━━━━━━━━━━━\\
case 'ban':
if (!isOwner) return reply(mess.only.ownerB)
...
//━━━━━━━━━━━━━━━[ STICKER ]━━━━━━━━━━━━━━━\\
			...
			Fiin.sendMessage(from, wkwk, sticker, {quoted: froxx})
			break
			case 'amongus':
			if (isBanned) return reply(mess.banned)
			random = Math.floor(Math.random() * 6) + 1
			...
			media = await Fiin.downloadAndSaveMediaMessage(encmedia)
			ran = getRandom('.png')
			...
			break
					...
					if (isBanned) return reply(mess.banned)
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await Fiin.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											Fiin.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: froxx})
											fs.unlinkSync(media)	
...
Fiin.sendMessage(from, mp4, video)
break
case 'mp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
Fiin.sendMessage(from, mp3, audio)
break
case 'ytmp3':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
Fiin.sendMessage(from, thum, image, { quoted: mek, caption: ini_txt })
res = await getBuffer(get.url_audio)
Fiin.sendMessage(from, res, audio)
break
case 'ytmp4':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
Fiin.sendMessage(from, res, video)
break
case 'tiktok':
if (isBanned) return reply(mess.banned)
reply(mess.wait)
...
reply(mess.wait)
              ...
              Fiin.sendMessage(from, ini_video, video, { quoted: froxx })
              break
          case 'tiktokmp3': 
          if (isBanned) return reply(mess.banned)
           reply(mess.wait)
              ...
              Fiin.sendMessage(from, data, audio, { quoted: froxx })
              break
              case 'ytplaymp3':
              await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=${ZeksKey}&q=${q}`)
		     .then(res => {
			  Fiin.sendMessage(from, { url: res.data.result.link }, 'audioMessage', { mimetype: 'audio/mp4', quoted: froxx, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.source }}})
})
              break
case 'lirik':
if (isBanned) return reply(mess.banned)
...
case 'bc': 
					...
					anu = await Fiin.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await Fiin.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							Fiin.sendMessage(_.jid, bc, image, {quoted:mek,caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})}
             reply('Sukses Boadcast')
             } else {
             for (let _ of anu) {
             Fiin.sendMessage(_.jid, 
			{"contentText": `*「 BROADCAST 」*\n\n${body.slice(4)}`,
			"footerText": 'Fiin-Botz by Fiin',
			"buttons": [
        ...
        if (isBanned) return reply(mess.banned)
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Fiin.downloadMediaMessage(encmedia)
             for (let i of anu) {
             	tes = `${body.slice(4)}`
             	Fiin.sendMessage(i.jid, bc, { contentText: `${tes}`, footerText: `_Broadcast*_`, buttons: [{buttonId: `${prefix}owner`,buttonText:{displayText: 'OWNER'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: bctlah, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             reply('Suksess Broadcast')
             } else {
             for (let i of anu) {
             	textt = `${body.slice(4)}`
             Fiin.sendMessage(i.jid, { contentText: `${textt}`, footerText: `_Broadcast*_`, buttons: [{buttonId: `${prefix}owner`,buttonText:{displayText: 'OWNER'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: bctlah, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')}
             reply('Sukses broadcast')}
             break
...
Fiin.sendMessage(from, buff, image, {quoted: froxx, caption : teks})
break
...
group = await Fiin.groupMetadata(from);
...
await Fiin.sendMessage(from, options, text)
break
case 'wa.me':
case 'wame':
if (isBanned) return reply(mess.banned)
...
const vcard = 'BEGIN:VCARD\n'  
            ...
            + 'END:VCARD'  
  Fiin.sendMessage(from, {displayname: "Fiin", vcard: vcard}, MessageType.contact, { quoted: mek})
...
{buttonId: `${prefix}script`, buttonText: { displayText: `SCRIPT`, }, type: 1, }
                 ...
                 if (isBanned) return reply(mess.banned)
					...
					Fiin.sendMessage(`${nomorowner}@s.whatsapp.net`, options, text, {quoted: mek})
...
if (isBanned) return reply(mess.banned)
					...
					if (args.length > 300) return Fiin.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
					stod = `${sender}`
					const repo = `*[REPORT]*\nNomor : @${stod.split('@')[0]}\nPesan : ${inipesannya}`
							var options = {
							text: repo,
      ...
      if (isBanned) return reply(mess.banned)
             ...
             let totalgerup = Fiin.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
        ...
        if (isBanned) return reply(mess.banned)
            ...
            var response = await Fiin.acceptInvite(codeInvite)
        ...
        if (isBanned) return reply(mess.banned)
             ...
             fak = Fiin.query({ json: ['action', 'invite', link],
             expect200: true })
...
//━━━━━━━━━━━━━━━[ GROUP ]━━━━━━━━━━━━━━━\\
 ...
 if (!isGroup) return reply(mess.only.group)
             ...
             let online = [...Object.keys(Fiin.chats.get(ido).presences), Fiin.user.jid]
             Fiin.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
  ...
  if (!isGroup) return reply(mess.only.group)
              ...
              Fiin.groupUpdateSubject(from, q)
  ...
  if (!isGroup) return reply(mess.only.group)
              ...
              Fiin.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
       ...
       if (isBanned) return reply(mess.banned)
           if (!isGroup) return reply(mess.only.group)
              ...
              let media = await Fiin.downloadMediaMessage(encmedia)
              Fiin.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
		...
		if (!isBotGroupAdmins) return reply(mess.only.Badmin)
		 ...
		 for (let mem of groupMembers) {
		 	...
		 	members_id.push(mem.jid)
		 		  ...
		 		  Fiin.groupDemoteAdmin(from, members_id)
	...
	if (isBanned) return reply(mess.banned)
		 		    	...
		 		    	for (let mem of groupMembers) {
		 		    	 	...
		 		    	 	members_id.push(mem.jid)
		 		    	 	 	   ...
		 		    	 	 	   Fiin.groupMakeAdmin(from, members_id)
             
		 		    	 	 	      break
case 'group':
if (isBanned) return reply(mess.banned)
if (!isGroup) return reply(mess.only.group)
    nthn = '©Created by Fiin'
...
if (isBanned) return reply(mess.banned)
      if (!mek.key.fromMe && !isGroupAdmins) return reply("Hanya Bisa Dilakukan Oleh Bot Dan Admin Group Saja");
        ...
        Fiin.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break; 
      case "opengc":
      if (isBanned) return reply(mess.banned)
  if (!mek.key.fromMe && !isGroupAdmins) return reply("Hanya Bisa Dilakukan Oleh Bot Dan Admin Group Saja");
        ...
        Fiin.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break; 
                case 'hidetag':        
   if (isBanned) return reply(mess.banned)                
					if (!isGroup) return reply(mess.only.group)
					...
					var lahh = await Fiin.groupMetadata(from)
					...
					Fiin.sendMessage(from, value, text, {quoted: ftag, contextInfo: { mentionedJid: mem }})
					break;
									case 'tagall':
									if (isBanned) return reply(mess.banned)
if (!isGroup) return reply(mess.only.group)
...
if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						Fiin.groupRemove(from, mentioned)
					} else {
						mentions(`Berhasil Promote @${mentioned[0].split('@')[0]} Sebagai Admin Group!`, mentioned, true)
						Fiin.groupMakeAdmin(from, mentioned)
					}
					break
				case 'demote':
				if (isBanned) return reply(mess.banned)
			if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						Fiin.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'add':
				if (isBanned) return reply(mess.banned)
			...
			if (!isGroup) return reply(mess.only.group)
                ...
                if (isBanned) return reply(mess.banned)
                 if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    linkgc = await Fiin.groupInviteCode(from)
                    reply('https://chat.whatsapp.com/'+linkgc)
                    break
                case 'leave':
            if (!isGroup) return reply(mess.only.group)
                    if (isGroupAdmins || isOwner) {
                    	Fiin.groupLeave(from)
                    } else {
...
if (isBanned) return reply(mess.banned)
        ...
        break;
                ...
                if (isBanned) return reply(mess.banned)
					...
					if (Number(args[0]) === 1) {
						...
						Fiin.sendMessage(from,`*ANTILINK* Sudah Aktif. Semua Member Dilarang Mengirim Link Group, Karena Bot Akan Mengeluarkan Target Jika Target Tersebut Mengirimkan Link Group`, text)
					} else if (Number(args[0]) === 0) {
        ...
        if (isBanned) return reply(mess.banned)
     if (!isGroup) return reply(mess.only.group)
					Fiin.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
case 'stikernowm': 
				case 'stickernowm':
				case 'snowm':
				if (isBanned) return reply(mess.banned)
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await Fiin.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
							.on('start', function (cmd) {
							...
							.on('end', function () {
								console.log('Finish')
								buffer = fs.readFileSync(ran)
								Fiin.sendMessage(from, buffer, sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
...
Fiin.sendMessage(from, 'Oke Sudah Aktif', text, {quoted: mek})
                  }
}
if (budy.startsWith('x')){
try {
return Fiin.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
